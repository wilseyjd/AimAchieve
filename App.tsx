...
function GoalsPage({ state, setState }: { state: AppState, setState: React.Dispatch<React.SetStateAction<AppState>> }) {
  ...
  const handleToggleKRStatus = (id: string, newStatus: 'active' | 'archived') => {
    setState(prev => ({
      ...prev,
      keyResults: prev.keyResults.map(kr => kr.id === id ? { ...kr, status: newStatus } : kr)
    }));
  };
  ...
  const handleApplyAI = () => {
    ...
    previewData.keyResults.forEach((krData: any) => {
      ...
      newKRs.push({
        ...
        currentValue: 0,
        unit: krData.unit,
        dueDate: undefined, // AI doesn't currently provide optional KR due dates
        status: 'active'
      });
